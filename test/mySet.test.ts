import mySet from "../src/mySet";
import { getUnion, getIntersection, getDifference, isSubsetOf } from "../src/mySet";
...
describe("集合的测试", () => {
    ...
    test("方法测试", () => {
        ...
        expect(set.delete(30)).toBe(true);
        expect(set.delete(30)).toBe(false);
...
describe("02 ES2015 Set 类测试", () => {
    ...
    test("求交集测试，setA < setB", () => {
        expect(getIntersection(setC, setD)).toEqual(new Set([2, 3]));
    });
    test("求差集测试", () => {
    ...
    test("求子集测试", () => {
        ...
        expect(isSubsetOf(setC, setD)).toBe(false);
    });
});
